...
from qa import initialize_qa, get_qa_system, ask_question, get_system_status
import psycopg2
from flask import (
    Flask,
    flash,
        ...
        if 'user_id' in session:
            try:
                ...
                if result and result[0]:
                    ...
                    logger.info(
                        f"Setting language to {
                            result[0]} from database for user {
                            session['user_id']}")
                else:
                    session['language'] = 'en'
                    logger.info(
                        f"No language found for user, defaulting to 'en'")
            except Exception as e:
                logger.error(
                    f"Error retrieving language from database: {
                        str(e)}")
                session['language'] = 'en'
        else:
            session['language'] = 'en'
    ...
    if language in LANGUAGES:
        ...
        if 'user_id' in session:
            ...
            try:
                ...
                conn.commit()
                logger.info(
                    f"Language updated to {language} for user {
                        session['user_id']}")
            except Exception as e:
                ...
                try:
                    ...
                    if isinstance(core_raw, list):
                        ...
                        if core_raw and isinstance(core_raw[0], dict):
                            ...
                            # Handle old format where it might be a list of
                            # strings
                            parsed = []
                    ...
                    logger.warning(
                        f"Error parsing core_elements for word {
                            w.get(
                                'id', 'unknown')}: {e}")
                    w['core_elements'] = []
            else:
...
def quiz(unit_id: int) -> Any:
    ...
    try:
        ...
        if attempt and attempt[1] is not None:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM quizzes WHERE unit_id = %s", (unit_id,))
            ...
            for i, result in enumerate(review_results):
                logger.info(
                    f"Question {
                        i +
                        1}: user_answer={
                        result['user_answer']}, correct_index={
        ...
        if request.method == 'POST':
            cursor.execute(
                "SELECT id, question, options, correct_answer FROM quizzes WHERE unit_id=%s",
                (unit_id,
                 ))
        ...
        # GET first attempt
        cursor.execute(
            "SELECT id, question, options FROM quizzes WHERE unit_id=%s", (unit_id,))
        questions = cursor.fetchall()
...
def ask_ai_enhanced() -> Any:
    """Simplified API endpoint for conversational chat."""
    if not session.get('authenticated'):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"AI response generated for user {
                    session.get(
                        'username',
        ...
        try:
            threading.Thread(
                target=save_qa_history_async,
                args=(session['user_id'], question, answer),
                daemon=True
            ).start()
        except Exception as db_error:
...
def ask_ai() -> Any:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"AI Question answered successfully for user {
                    session.get(
                        'username',
                        'unknown')}")
            ...
            for submission in submissions:
                ...
                if file_path:
                    ...
                    if os.path.exists(real_path):
                        ...
                        file_name = f"Unit{
                            submission['unit_id']}_{
                            submission['username']}_{file_path}"
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if not core_elements_list and 'core_elements' in request.form:
                ...
                if textarea_value:
                    # Parse textarea format: "Core Element - Everyday Object"
                    # per line
                    core_elements_list = []
...
def admin_add_quiz() -> Any:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            options = [data[f'option{i}']
                       for i in range(1, 4)]  # Updated to 5 options
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if allowed_file(file.filename):
                ...
                title = os.path.splitext(original_filename)[0].replace(
                    '_', ' ').replace('-', ' ').title()
...
def admin_manage_content() -> Any:
    ...
    try:
        ...
        # Get all quizzes
        cursor.execute(
            "SELECT id, unit_id, question FROM quizzes ORDER BY unit_id, id")
        quizzes = cursor.fetchall()

        # Get all materials
        cursor.execute(
            "SELECT id, unit_id, title FROM materials ORDER BY unit_id, id")
        materials = cursor.fetchall()

        # Get all videos
        cursor.execute(
            "SELECT id, unit_id, title FROM videos ORDER BY unit_id, id")
        videos = cursor.fetchall()

        # Get all projects
        cursor.execute(
            "SELECT id, unit_id, title FROM projects ORDER BY unit_id, id")
        projects = cursor.fetchall()

        # Get all AI vocabulary words with section
        cursor.execute(
            "SELECT id, unit_id, word, section FROM words ORDER BY unit_id, section, id")
...
def admin_export_users() -> Any:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, username, email, language, email_verified FROM users ORDER BY id")
        users = cursor.fetchall()
        cursor.close()
        release_db_connection(conn)
...
def view_submission(submission_id: int) -> Any:
    ...
    try:
        ...
        download_name = f"Unit{
            submission['unit_id']}_{
            submission['username']}_{file_name}"
        ...
        if os.path.exists(file_path):
            ...
            if remaining_files:
                ...
                qa_system = initialize_qa(
                    ...
                    llama_model_path=LLAMA_MODEL_PATH if 'LLAMA_MODEL_PATH' in globals() else None)
                flash(
                    f"Document deleted. Q&A system updated with {
                        len(remaining_files)} remaining documents.",
                    "success")
            else:
...
def admin_rebuild_qa() -> Any:
    ...
    try:
        ...
        if supported_files:
            ...
            flash(
                f"Q&A system rebuilt successfully with {
                    len(supported_files)} documents",
                "success")
        else:
...
def debug_submission(submission_id: int) -> Any:
    ...
    try:
        ...
        if not file_exists:
            debug_info.append(
                f"UPLOAD_FOLDER is configured as: {UPLOAD_FOLDER}")
            debug_info.append(
                f"UPLOAD_FOLDER absolute path: {
                    os.path.abspath(UPLOAD_FOLDER)}")
            debug_info.append(
                f"Files in upload folder: {
...
if __name__ == '__main__':
    try:
        initialize_enhanced_qa_system()  # Changed from initialize_simple_qa_system()
        init_db_pool()
